class TaxonsTreeComponentFaculties2 < ViewComponent::Base
    ...
    def tree(root_taxon:, base_class:, max_level:)
        ...
        content_tag :div, class: "container4 semi-transparent-un reveal active", id: 'ar' do
              ...
              link_to(helpers.taxon_seo_url(taxon), class: '') do
                content_tag(:div, class: 'inherit') do
                  (taxon.icon.attached? ? content_tag(:div, class: '') do
                    image_tag(taxon.icon.url, class: 'box-d-3-fa', alt: '')
                  end : '') +
                  content_tag(:div, class: 'box-d-2-fa grid') do
                    content_tag(:span, 'read more', class: 'text-fa black')
                  end
                end
              end +

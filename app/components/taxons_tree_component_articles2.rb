class TaxonsTreeComponentArticles2 < ViewComponent::Base
              ...
              link_to(helpers.taxon_seo_url(taxon), class: '') do
                content_tag(:div, class: 'inherit') do
                  (taxon.icon.attached? ? content_tag(:div, class: '') do
                    image_tag(taxon.icon.url, class: 'box-d-3-ar', alt: '')
                  end : '') +
                  content_tag(:div, class: 'box-d-2-ar grid') do
                    content_tag(:span, 'read more', class: 'text-ar black')
                  end
                end
              end +
